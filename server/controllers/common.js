...
const googleSpreadSheet = require('../helpers/googlespreadsheet/googleSheetAPI')
const rcmdProjectsAI = require('../helpers/rcmdaiAPI/rcmdAIapi')
 ...
 * IA Recomanador
 */
router.post(['/r', '/r/*'], async (req, res, next) => {
  const pageArgs = pageHelper.parsePath(req.path, { stripExt: true })
  if (!WIKI.auth.checkAccess(req.user, [
        'manage:system','manage:projectsAI'
   ]))
   {
    _.set(res.locals, 'pageMeta.title', 'Unauthorized')
    return res.render('unauthorized', { action: 'rcmd' })
   } 
...
router.get(['/r', '/r/*'], async (req, res, next) => {
  ...
  _.set(res.locals, 'pageMeta.title', 'Recomendador')
  const credentials = JSON.parse(WIKI.config.rcmd.credits);

  switch (pageArgs.path) {

  case 'create/data-sheet':
    const result = await googleSpreadSheet.getSpreadSheetValuesAPI(WIKI.config.rcmd.spreadSheetId,credentials,['sectores','tecnologias', 'numempleados', 'areassostenibilidad', 'perfilestudiante', 'empleos', 'trabajos', 'horas'])
    const jsonString = JSON.stringify(Object.assign({}, result))
    res.json(JSON.parse(jsonString));
    break;

  case 'create/data-sheet-project':
    const resultS = await googleSpreadSheet.getSpreadSheetValuesAPI(WIKI.config.rcmd.spreadSheetId,credentials,['sectores', 'perfilestudiante','trabajos'])
