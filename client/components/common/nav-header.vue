    ...
    v-layout(row)
      ...
      v-flex(md4, v-if='$vuetify.breakpoint.mdAndUp')
        v-toolbar.nav-header-inner(color='black', dark, flat)
          ...
          template(v-if='hasProjectsAIPermission && path && mode !== `edit`')
            v-tooltip(bottom)
              template(v-slot:activator='{ on }')
                v-btn(icon, tile, height='64', v-on='on', @click='projectsAI', :aria-label='"Project AI suggester"')
                  v-icon(color='grey') mdi-auto-fix
              span {{ 'Projects AI suggester' }}
            v-divider(vertical)
      v-flex(xs7, md4)
...
export default {
  ...
  computed: {
    ...
    hasProjectsAIPermission () {
      return this.hasAdminPermission || _.intersection(this.permissions, ['manage:projectsAI']).length > 0
    },
