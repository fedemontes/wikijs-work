        ...
        v-container(fluid,grid-list-lg)
          v-card.mt-3.animated.fadeInUp(:class='$vuetify.theme.dark ? `grey darken-3-d5` : `grey lighten-2`')
           v-row.pa-2
            .pa-3.d-flex.align-center(:class='$vuetify.theme.dark ? `grey darken-3-d5` : `grey lighten-2`')
             ...
             v-col
              v-btn.align-center(color='success', large, active, @click='exportDataToCsv')
          ...
          v-data-table(
           ...
           template(slot='item', slot-scope='props')
            tr.is-clickable(:active='props.selected', @click='$router.push(`/projects/` + props.item.id)')
                ...
                td.truncate-options
                  div.options
                   v-chip.mt-1(color='primary',v-for='(value, key) of props.item.arguments')
                    span {{ key +":"+ value }}
  ...
  created () {
   axios.get('/r/create/data-sheet-project')
      .then(response => {
        ...
        this.dataSheet.trabajos.unshift("");
