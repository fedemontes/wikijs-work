      ...
      vue-scroll(:ops='scrollStyle')
        v-list.radius-0(dense, nav)
          ...
          template(v-if='hasPermission([`manage:system`, `manage:navigation`, `write:pages`, `manage:pages`, `delete:pages`])')
            ...
            v-list-item(to='/rcmd/', color='primary', v-if='hasPermission([`manage:system`, `manage:projectsAI`])')
              v-list-item-avatar(size='24', tile): v-icon mdi-auto-fix
              v-list-item-title Recomendador 
...
const router = new VueRouter({
  ...
  routes: [
    ...
    { path: '/rcmd', component: () => import(/* webpackChunkName: "admin" */ './admin/admin-rcmd.vue') },
